...
public class GameOfLife {
	...
	private static void test2(String fileName, int i, int j) {
		int[][] board = read(fileName);
		print(board);
		System.out.println("Current cell value: " + board[i][j]);
		System.out.println("Number of cell neighbors: " + count(board, i, j));
		System.out.println("Next gen cell should be: " + cellValue(board, i, j));
	...
	public static int[][] read(String fileName) {
		...
		{
			String line = "." + in.readLine(); // put 0 in the first cell
	...
	public static int cellValue(int[][] board, int i, int j) {
		int numOfaliveNeighbors = count(board, i, j);
		if(board[i][j] == 1)
		{
	...
	public static int count(int[][] board, int i, int j) {
		...
		for(int rows = i - 1; rows <= i + 1; rows++)
		{
			...
			{
				if((rows != i || cols != j) && board[rows][cols] == 1)
					counter++;
			}
		}
		return counter;
